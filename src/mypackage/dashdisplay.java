...
public class dashdisplay extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    try{  
			...
			String q6="Select * from billtype";
			PreparedStatement st6=(PreparedStatement) con.prepareStatement(q6);
			
			ResultSet g6=st6.executeQuery();
			while(g6.next()){
				
				
				type.put(g6.getString("type"));
			}
			g6.close();
			...
			String s=null;
			String q7="Select status from group_member where email=?";
			PreparedStatement st7=(PreparedStatement) con.prepareStatement(q7);
			st7.setString(1, keyid);
			ResultSet g7=st7.executeQuery();
			while(g7.next()){
				s=g7.getString("status");
			...
			String q2="SELECT member_name from  group_member where gname=? and status in('A','RL','RR')";
			...
			String q3="SELECT transaction.purpose,transaction.bill_id,transaction.total_amt,transaction.date,transaction.votes from transaction inner join bills on transaction.bill_id=bills.bill_id where transaction.gname=? and transaction.status='P' and bills.member_id=?";
			PreparedStatement st3=(PreparedStatement) con.prepareStatement(q3);
		st3.setString(1, gname);
		st3.setString(2, keyid);
