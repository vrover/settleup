...
public class pollingyes extends HttpServlet {
	...
	//	System.out.println(billid);
		...
		//	System.out.println(vote);
			...
			String q11="Select status from transaction where bill_id=? and gname=?";
			PreparedStatement s11=(PreparedStatement) con.prepareStatement(q11);
			s11.setInt(1, billid);
			s11.setString(2,gname);
			ResultSet grp11=s11.executeQuery();
			while(grp11.next()){
				status=grp11.getString("status");	
			}
			
			
			String q12="Select payment_type from bills where bill_id=? and gname=?";
			PreparedStatement s12=(PreparedStatement) con.prepareStatement(q12);
			s12.setInt(1, billid);
			s12.setString(2,gname);
			ResultSet grp12=s12.executeQuery();
			while(grp12.next()){
				type=grp12.getString("payment_type");	
			}
			
			
			System.out.println("fraction "+((float)newvote/(float)outoff));
			if(((float)newvote/(float)outoff)>=0.5 && !status.equals("A")){
